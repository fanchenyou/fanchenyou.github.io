---
title: Gallery
layout: page
description: Lab moments and activities
permalink: "/gallery/"
---

<div class="gallery-page-intro">
  <p>Team photos, workshop snapshots, campus visits, and moments that capture the spirit of FancyAI Lab.</p>
</div>

<div class="gallery-featured-grid">
  <figure class="gallery-feature-card large">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab group photo" />
    <figcaption>
      <span>Group Photo</span>
      <strong>FancyAI Lab annual team portrait</strong>
    </figcaption>
  </figure>
  <figure class="gallery-feature-card tall">
    <img src="{{ '/images/team_pic/2025-04-27.jpg' | relative_url }}" alt="FancyAI Lab campus activity" />
    <figcaption>
      <span>Lab Activity</span>
      <strong>2022级毕业合照</strong>
    </figcaption>
  </figure>
  <figure class="gallery-feature-card">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab members together" />
    <figcaption>
      <span>Research Community</span>
      <strong>A collaborative and supportive team culture</strong>
    </figcaption>
  </figure>
</div>

<div class="gallery-masonry-grid">
  <figure class="gallery-tile">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab team photo one" />
  </figure>
  <figure class="gallery-tile">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab team photo two" />
  </figure>
  <figure class="gallery-tile">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab team photo three" />
  </figure>
  <figure class="gallery-tile">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab team photo four" />
  </figure>
  <figure class="gallery-tile">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab team photo five" />
  </figure>
  <figure class="gallery-tile">
    <img src="{{ '/images/1686104359209136.jpg' | relative_url }}" alt="FancyAI Lab team photo six" />
  </figure>
</div>
